...
| 28 | headless | `-p` flag, piping, scripting | `07-voice-input/voice_claude.py` (subprocess pattern) | **Partial** — no `-p` flag shell pipeline demo; Python subprocess ≠ `-p` CLI flag |
...
| 33 | plugins | Creating and installing plugins | `08-superpowers/` (superpowers plugin analysis) | **Partial** — no original plugin built |
...
|----------|--------|-----|--------|-------|
| 1 | §28 | Headless / piping (`-p` flag) | Low | High — unlocks CI automation without the SDK |
| 2 | §27 | Agent teams (parallel sessions) | Medium | High — completes the multi-agent picture |
| 3 | §39 | GitHub Code Review (automatic) | Low | High — distinct from @claude workflow |
| 4 | §08+§09 | CLAUDE.local.md, @-refs, context cheatsheet | Low | Medium — foundational but CLAUDE.md already in use |
| 5 | §17 | Checkpointing | Low | Medium — safety net for long sessions |
| 6 | §38 | Sandboxing | Medium | Low–Medium — production safety; overkill for solo dev |
