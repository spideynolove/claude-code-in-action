...
Planning Mode + Thinking Modes
=> 
```
What if I forget to "turn on Shift + Tab twice" or I'm afraid to type "Think" (or its other 4 levels). I even coded a `sequential thinking mcp` myself. Sometimes I feel like it doesn't matter if I "turn on Shift + Tab twice" or not. There is an overlap between `mcp` and the "think" functions as mentioned, is there any way to coordinate it harmoniously? "Think" + "PLan" + "Remember" -> "1 insight" | continue "Think" + "PLan" + "Remember" -> "another insight" ... | "many insights" into a "real sequence insight" strong enough to implement from start to finish without "losing | getting lost | forgetting" the context when working even though the context becomes large
```
